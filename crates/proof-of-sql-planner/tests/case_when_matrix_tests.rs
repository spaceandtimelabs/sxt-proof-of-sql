...
fn cats_table(alloc: &Bump) -> IndexMap<TableRef, Table<'_, DoryScalar>> {
    indexmap! {
...
fn test_case_when_numeric_branches() {
    ...
    SELECT CASE WHEN id = 1 THEN 10 WHEN id = 2 THEN 20 WHEN id = 3 THEN 30 ELSE 0 END AS v FROM cats;
    SELECT CASE WHEN id = 1 THEN 10 WHEN id = 2 THEN 20 WHEN id = 3 THEN 30 WHEN id = 4 THEN 40 ELSE 0 END AS v FROM cats;
    ...
    let expected_results = vec![
        ...
        // 2 conditions -> 1 committed guard
        owned_table([bigint("v", [10_i64, 20, 0, 0, 0])]),
        // 3 conditions -> 2 committed guards; g_2 depends on g_0 and g_1
        owned_table([bigint("v", [10_i64, 20, 30, 0, 0])]),
        // 4 conditions -> 3 committed guards
        owned_table([bigint("v", [10_i64, 20, 30, 40, 0])]),
...
// Varchar branches prove and verify: the prover selects real strings row-wise.
#[test]
fn test_case_when_varchar_branches() {
    use proof_of_sql::base::database::owned_table_utility::*;
    let sql = "SELECT CASE WHEN weight > 5.0 THEN 'big' ELSE 'small' END AS size FROM cats;
    SELECT CASE WHEN adopted THEN name ELSE 'unadopted' END AS label FROM cats;";
    let expected_results = vec![
        // weights: 14.5, 7.5, 2.0, 4.5, 5.5
        owned_table([varchar("size", ["big", "big", "small", "small", "big"])]),
        owned_table([varchar(
            "label",
            ["Chloe", "Margaret", "unadopted", "unadopted", "Prudence"],
        )]),
    ];
    assert_case_queries_prove_and_verify(sql, &expected_results);
}
...
fn test_unsupported_case_when_variants() {
    ...
    // Varchar branches are supported: the prover selects real strings row-wise.
    let statements = Parser::parse_sql(
        &GenericDialect {},
        "SELECT CASE WHEN weight > 5.0 THEN 'big' ELSE 'small' END FROM cats",
    )
    .unwrap();
    assert!(sql_to_proof_plans(&statements, &accessor, &config).is_ok());
