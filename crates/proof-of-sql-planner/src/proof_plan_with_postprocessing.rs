        ...
        Err(_err) => {
            match plan {
                ...
                LogicalPlan::Projection(Projection { input, expr, .. }) => {
                    ...
                    let postprocessing = SelectPostprocessing::new(expr.to_vec());
...
mod tests {
    ...
    use datafusion::{
        ...
        logical_expr::{
            ...
            Aggregate, Expr, TableScan, TableSource,
        },
        physical_plan,
    };
    use indexmap::indexmap;
    ...
    fn TABLE_SOURCE() -> Arc<dyn TableSource> {
        Arc::new(PoSqlTableSource::new(vec![
            ...
            ColumnField::new("num_of_letters".into(), ColumnType::Int64),
    ...
    fn we_can_convert_logical_plan_to_proof_plan_with_postprocessing() {
            ...
            Projection::try_new(
                vec![
                    Expr::Column(Column::new(
                        None::<TableReference>,
                        "COUNT(Int64(1)) * Int64(2)".to_string(),
                    ))
        ...
        let expected_plan = DynProofPlan::new_group_by(
            ...
            vec![],
            "num_languages_using_abjad".into(),
            TableExpr {
        ...
        let expected_postprocessing = SelectPostprocessing::new(vec![
            ...
            .alias("tiwce_num_languages_using_abjad"),
            df_column("languages", "language_family"),
        ]);
        ...
        assert_eq!(
            result.postprocessing().clone().unwrap(),
