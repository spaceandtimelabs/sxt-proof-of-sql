...
fn big_scalars_with_small_additive_inverses_are_encoded_as_negative_zigzag_values() {
    ...
    // encoded as 2 * y - 1 = 1
    assert!((-Scalar::from(1_u32)).zigzag() == U256::from_words(1, 0));
    ...
    // encoded as 2 * y - 1 = 3
    assert!((-Scalar::from(2_u32)).zigzag() == U256::from_words(3, 0));

    for y in 1..1000_u128 {
        // since x > y, where x + y = 0, the ZigZag value is encoded as 2 * y - 1
        assert!((-Scalar::from(y)).zigzag() == U256::from_words(2 * y - 1, 0));
    }
}
