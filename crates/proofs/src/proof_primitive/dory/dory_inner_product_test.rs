use super::{
    ...
    ProverSetup, ProverState, VerifierSetup, G1, GT,
};
...
fn we_can_prove_and_verify_a_dory_inner_product() {
    ...
    let prover_setup = ProverSetup::new(&Gamma_1, &Gamma_2, nu);
    let verifier_setup = VerifierSetup::new(&Gamma_1, &Gamma_2, nu);
...
fn we_can_prove_and_verify_a_dory_inner_product_for_multiple_nu_values() {
    ...
    let prover_setup = ProverSetup::new(&Gamma_1, &Gamma_2, max_nu);
    let verifier_setup = VerifierSetup::new(&Gamma_1, &Gamma_2, max_nu);
...
fn we_fail_to_verify_when_the_setups_differ() {
    ...
    let verifier_setup = VerifierSetup::new(&Gamma_1_wrong, &Gamma_2_wrong, nu);
    let (v1, v2) = rand_G_vecs(nu, &mut rng);
    let prover_state = ProverState::new(v1, v2, nu);
    let verifier_state = prover_state.calculate_verifier_state(&prover_setup);
