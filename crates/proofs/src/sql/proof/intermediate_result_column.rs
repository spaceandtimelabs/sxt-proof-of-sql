...
pub trait IntermediateResultColumn {
...
pub struct DenseIntermediateResultColumn<'a, T: VarInt> {
    data: &'a [T],
}

impl<'a, T: VarInt> DenseIntermediateResultColumn<'a, T> {
...
impl<'a, T: VarInt> IntermediateResultColumn for DenseIntermediateResultColumn<'a, T> {
    fn num_bytes(&self, selection: &[u64]) -> usize {
        let mut res = 0;
