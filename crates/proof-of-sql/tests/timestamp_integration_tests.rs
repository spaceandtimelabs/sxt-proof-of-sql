...
fn we_can_prove_a_basic_query_containing_rfc3339_timestamp_with_dory() {
    ...
    let owned_table_result = proof
        ...
        .unwrap()
        .table;
    let expected_result = owned_table([timestamp("times", ["1970-01-01T00:00:00Z".to_string()])]);
...
fn run_timestamp_query_test(
    ...
    expected_timestamps: Vec<&str>, // Expected timestamps to match the query result
) {
    ...
    accessor.add_table(
        "sxt.table".parse().unwrap(),
        owned_table([timestamp(
            "times",
            test_timestamps.iter().map(|f| f.to_string()),
        )]),
        0,
    );
    ...
    let owned_table_result = proof
        ...
        .unwrap()
        .table;
    let expected_result = owned_table([timestamp(
        "times",
        expected_timestamps.iter().map(|f| f.to_string()),
    )]);
...
mod tests {
    ...
    fn test_basic_timestamp_query() {
        // Updated test_timestamps to include one second before, during, and after the Unix epoch
        ...
        // Expected timestamps should only include the Unix epoch
        let expected_timestamps = vec![
            "1970-01-01T00:00:00Z", // The Unix epoch
        ];

        // Select only the record that matches the Unix epoch
        ...
        run_timestamp_query_test(
            "SELECT * FROM table WHERE times > timestamp '1970-01-01T00:00:00Z';",
            test_timestamps.clone(),
            vec!["1970-01-01T00:00:01Z"],
        );

        run_timestamp_query_test(
            "SELECT * FROM table WHERE times < timestamp '1970-01-01T00:00:00Z';",
            test_timestamps,
            vec!["1969-12-31T23:59:59Z"],
        );
    }
