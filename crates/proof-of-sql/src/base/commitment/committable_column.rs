...
pub enum CommittableColumn<'a> {
    ...
    TimestampTZ(PoSQLTimeZone, &'a [i64]),
}
...
impl<'a> CommittableColumn<'a> {
    ...
    pub fn len(&self) -> usize {
        match self {
            ...
            CommittableColumn::TimestampTZ(_, col) => col.len(),
        }
    }
...
impl<'a> From<&CommittableColumn<'a>> for ColumnType {
    fn from(value: &CommittableColumn<'a>) -> Self {
        match value {
            ...
            CommittableColumn::TimestampTZ(tz, _) => ColumnType::TimestampTZ(*tz),
        }
    }
}
...
impl<'a, S: Scalar> From<&Column<'a, S>> for CommittableColumn<'a> {
    fn from(value: &Column<'a, S>) -> Self {
        match value {
            ...
            Column::TimestampTZ(tz, times) => CommittableColumn::TimestampTZ(*tz, times),
        }
    }
}
...
impl<'a, S: Scalar> From<&'a OwnedColumn<S>> for CommittableColumn<'a> {
    fn from(value: &'a OwnedColumn<S>) -> Self {
        match value {
            ...
            OwnedColumn::TimestampTZ(tz, times) => {
                CommittableColumn::TimestampTZ(*tz, times as &[_])
            }
        }
    }
}
...
impl<'a, 'b> From<&'a CommittableColumn<'b>> for Sequence<'a> {
    fn from(value: &'a CommittableColumn<'b>) -> Self {
        match value {
            ...
            CommittableColumn::TimestampTZ(_, times) => Sequence::from(*times),
        }
    }
}
...
mod tests {
    ...
    fn we_can_get_type_and_length_of_timestamp_column() {
        // empty case
        let committable_column = CommittableColumn::TimestampTZ(PoSQLTimeZone::Utc, &[]);
        ...
        let committable_column = CommittableColumn::TimestampTZ(PoSQLTimeZone::Utc, &[12, 34, 56]);
    ...
    fn we_can_convert_from_borrowing_timestamp_column() {
        ...
        let from_borrowed_column = CommittableColumn::from(
            &Column::<Curve25519Scalar>::TimestampTZ(PoSQLTimeZone::Utc, &[]),
        );
        assert_eq!(
            from_borrowed_column,
            CommittableColumn::TimestampTZ(PoSQLTimeZone::Utc, &[])
        );

        // non-empty case
        let timestamps = [1625072400, 1625076000, 1625083200];
        let from_borrowed_column = CommittableColumn::from(
            &Column::<Curve25519Scalar>::TimestampTZ(PoSQLTimeZone::Utc, &timestamps),
        );
        assert_eq!(
            from_borrowed_column,
            CommittableColumn::TimestampTZ(PoSQLTimeZone::Utc, &timestamps)
        );
    }
    ...
    fn we_can_convert_from_owned_timestamp_column() {
        // empty case
        let owned_column =
            OwnedColumn::<Curve25519Scalar>::TimestampTZ(PoSQLTimeZone::Utc, Vec::new());
        let from_owned_column = CommittableColumn::from(&owned_column);
        assert_eq!(
            from_owned_column,
            CommittableColumn::TimestampTZ(PoSQLTimeZone::Utc, &[])
        ...
        let owned_column =
            OwnedColumn::<Curve25519Scalar>::TimestampTZ(PoSQLTimeZone::Utc, timestamps.clone());
        let from_owned_column = CommittableColumn::from(&owned_column);
        assert_eq!(
            from_owned_column,
            CommittableColumn::TimestampTZ(PoSQLTimeZone::Utc, &timestamps)
    ...
    fn we_can_commit_to_timestamp_column_through_committable_column() {
        ...
        // Non-empty case
        let timestamps = [1625072400, 1625076000, 1625083200];
        let committable_column = CommittableColumn::TimestampTZ(PoSQLTimeZone::Utc, &timestamps);
