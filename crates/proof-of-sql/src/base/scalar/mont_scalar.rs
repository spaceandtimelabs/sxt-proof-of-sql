...
impl<T: MontConfig<4>> MontScalar<T> {
    ...
    /// Convenience function for converting a slice of `ark_curve25519::Fr` into a vector of `TestScalar`. Should not be used outside of tests.
    #[cfg(test)]
    pub fn wrap_slice(slice: &[Fp256<MontBackend<T, 4>>]) -> Vec<Self> {
        slice.iter().copied().map(Self).collect()
    }
    /// Convenience function for converting a slice of `TestScalar` into a vector of `ark_curve25519::Fr`. Should not be used outside of tests.
    #[cfg(test)]
