...
fn we_can_filter_nullable_columns_with_null_operands() {
    ...
    let nullable_varchar =
        NullableColumn::with_presence(varchar_column, Some(&null_presence)).unwrap();
    ...
    assert_eq!(
        result[1],
        Column::VarChar((&["2", "4"], &["2".into(), "4".into()]))
    );
...
fn we_can_filter_complex_null_cases_with_multiple_where_clauses() {
    ...
    // Create nullable columns
    let nullable_value_a =
        NullableColumn::with_presence(value_a_column, Some(&value_a_presence)).unwrap();
    let nullable_name_b =
        NullableColumn::with_presence(name_b_column, Some(&name_b_presence)).unwrap();
    let nullable_score_c =
        NullableColumn::with_presence(score_c_column, Some(&score_c_presence)).unwrap();
    ...
    assert_eq!(
        result2[2],
        Column::VarChar((&["Charlie", "Frank"], &["Charlie".into(), "Frank".into()]))
    );
    assert_eq!(result2[3], Column::Scalar(&[4i64.into(), 4i64.into()]));
