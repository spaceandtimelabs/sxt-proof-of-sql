...
fn we_can_perform_null_operations_with_where_clause_in_three_valued_logic() {
    use crate::base::database::{
        ...
        owned_table_utility::{
            bigint_values, boolean_values, nullable_column_pair, owned_table_with_nulls,
            varchar_values,
        },
    };
    ...
    let where_result_1: Vec<bool> = a_eq_50
        .presence
        .unwrap()
        .iter()
    ...
    let where_result_2: Vec<bool> = and_result
        .presence
        .unwrap()
        .iter()
    ...
    assert_eq!(
        where_result_2,
        vec![false, false, false, false, false, true, false, false, true, false]
    );
    ...
    let c_is_null: Vec<bool> = c_presence
        .as_ref()
        .unwrap()
        .iter()
        .map(|present| !present)
    ...
    let c_is_null_col = OwnedNullableColumn::new(OwnedColumn::Boolean(c_is_null));

    // OR operation
    let or_result = a_lt_30.element_wise_or(&c_is_null_col).unwrap();

    // Extract rows that satisfy the WHERE clause
    let where_result_3: Vec<bool> = or_result
        .presence
        .unwrap()
        .iter()
    ...
    assert_eq!(
        where_result_3,
        vec![true, true, false, false, true, true, false, false, true, false]
    );
    ...
    let e_is_not_null: Vec<bool> = e_presence.as_ref().unwrap().clone();

    // Convert e_is_not_null to a nullable column
    let e_is_not_null_col = OwnedNullableColumn::new(OwnedColumn::Boolean(e_is_not_null));

    // AND operation for the final result
    let final_result = or_part.element_wise_and(&e_is_not_null_col).unwrap();

    // Extract rows that satisfy the WHERE clause
    let where_result_4: Vec<bool> = final_result
        .presence
        .unwrap()
        .iter()
