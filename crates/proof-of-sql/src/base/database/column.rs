...
impl<'a, S: Scalar> Column<'a, S> {
    ...
    pub fn column_type(&self) -> ColumnType {
        match self {
            ...
            Self::TimestampTZ(timezone, _) => ColumnType::TimestampTZ(*timezone),
        }
    }
    ...
    pub fn len(&self) -> usize {
        match self {
            ...
            Self::TimestampTZ(_, col) => col.len(),
        }
    }
    ...
    pub fn from_literal_with_length(
        ...
        match literal {
            ...
            LiteralValue::TimestampTZ(tz, value) => {
                Column::TimestampTZ(*tz, alloc.alloc_slice_fill_copy(length, *value))
            }
    ...
    pub(crate) fn scalar_at(&self, index: usize) -> Option<S> {
        (index < self.len()).then_some(match self {
            ...
            Self::TimestampTZ(_, col) => S::from(col[index]),
        })
    }
...
impl From<&ColumnType> for DataType {
    fn from(column_type: &ColumnType) -> Self {
        match column_type {
            ...
            ColumnType::TimestampTZ(timezone) => DataType::Timestamp(
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ColumnType::TimestampTZ(timezone) => {
                write!(f, "TIMESTAMP(TIMEZONE: {timezone})")
            }
        }
    }
}
...
mod tests {
    ...
    fn column_type_serializes_to_string() {
        let column_type = ColumnType::TimestampTZ(PoSQLTimeZone::Utc);
        let serialized = serde_json::to_string(&column_type).unwrap();
        assert_eq!(serialized, r#"{"TimestampTZ":"Utc"}"#);
    ...
    fn we_can_deserialize_columns_from_valid_strings() {
        let expected_column_type = ColumnType::TimestampTZ(PoSQLTimeZone::Utc);
        let deserialized: ColumnType = serde_json::from_str(r#"{"TimestampTZ":"Utc"}"#).unwrap();
