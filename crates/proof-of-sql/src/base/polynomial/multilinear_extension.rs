...
impl<S: Scalar> MultilinearExtension<S> for Column<'_, S> {
    fn inner_product(&self, evaluation_vec: &[S]) -> S {
        match self {
            ...
            Column::TimestampTZ(_, c) => c.inner_product(evaluation_vec),
        }
    }
    ...
    fn mul_add(&self, res: &mut [S], multiplier: &S) {
        match self {
            ...
            Column::TimestampTZ(_, c) => c.mul_add(res, multiplier),
        }
    }
    ...
    fn to_sumcheck_term(&self, num_vars: usize) -> Rc<Vec<S>> {
        match self {
            ...
            Column::TimestampTZ(_, c) => c.to_sumcheck_term(num_vars),
        }
    }
    ...
    fn id(&self) -> *const c_void {
        match self {
            ...
            Column::TimestampTZ(_, c) => MultilinearExtension::<S>::id(c),
        }
    }
}
