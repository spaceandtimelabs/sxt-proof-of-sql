...
pub fn get_word_counts<'a>(alloc: &'a Bump, columns: impl Iterator<Item = Vec<u8>>) -> &'a [i64]{
...
mod tests{
    ...
    use crate::base::{byte::{byte_matrix_utils::{compute_varying_byte_matrix, get_word_counts}, ByteDistribution}, scalar::{test_scalar::TestScalar, Scalar}};

    #[test]
    fn we_can_compute_varying_byte_matrix_for_small_scalars() {
        let scalars: Vec<TestScalar> = [1, 2, 3, 255, 256, 257].iter().map(TestScalar::from).collect();
        let byte_distribution = ByteDistribution::new::<TestScalar, _>(&scalars);
        let varying_columns = compute_varying_byte_matrix::<TestScalar>(&scalars, &byte_distribution).collect::<Vec<_>>();
    ...
    fn we_can_compute_varying_byte_matrix_for_large_scalars() {
        let scalars = vec![TestScalar::MAX_SIGNED, TestScalar::from(u64::MAX), -TestScalar::ONE];
        let byte_distribution = ByteDistribution::new::<TestScalar, _>(&scalars);
        let varying_columns = compute_varying_byte_matrix::<TestScalar>(&scalars, &byte_distribution).collect::<Vec<_>>();
