...
pub fn compute_varying_bit_matrix<S: Scalar>(
    vals: &[S],
    dist: &ByteDistribution,
) -> Vec<Vec<u8>> {
    let number_of_scalars = vals.len();
    let bit_masks = vals.iter().copied().map(make_bit_mask);
    dist.varying_byte_indices().map(|start_index| bit_masks.clone().map(|bit_mask| {
        ...
        shifted_byte
    })).fold(vec![Vec::new(); dist.varying_byte_indices().count()], |acc: Vec<Vec<u8>>, shifted_bytes| {
        acc.into_iter().zip(shifted_bytes).map(|(v, b)| v.into_iter().chain([b].into_iter()).collect_vec()).collect_vec()
    })
}