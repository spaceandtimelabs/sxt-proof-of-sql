...
use crate::base::{
    database::{order_by_util::compare_indexes_by_owned_columns_with_direction, OwnedTable},
...
pub struct OrderByPostprocessing {
    index_direction_pairs: Vec<(usize, bool)>,
}

impl OrderByPostprocessing {
    /// Create a new `OrderByPostprocessing` node.
    #[must_use]
    pub fn new(index_direction_pairs: Vec<(usize, bool)>) -> Self {
        Self {
