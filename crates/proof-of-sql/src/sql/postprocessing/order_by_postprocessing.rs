...
use crate::base::{
    database::{order_by_util::OrderIndexDirectionPairs, OwnedColumn, OwnedTable},
    map::IndexMap,
...
impl<S: Scalar> PostprocessingStep<S> for OrderByPostprocessing {
    ...
    fn apply(&self, owned_table: OwnedTable<S>) -> PostprocessingResult<OwnedTable<S>> {
        ...
        let order_by_column_names: Vec<_> = self
            .index_direction_pairs
            .iter()
            .map(|(index, _)| owned_table.column_names().nth(*index).unwrap().clone())
            .collect();
        ...
        let compare_with_nulls = |a: &usize, b: &usize| {
            for (idx, (col, is_asc)) in column_direction_pairs.iter().enumerate() {
                ...
                let a_is_null = presence_for_order_by
                    .get(&idx)
                    .is_some_and(|presence| !presence[*a]);

                let b_is_null = presence_for_order_by
                    .get(&idx)
        ...
        let permutation =
            Permutation::unchecked_new_from_cmp(owned_table.num_rows(), compare_with_nulls);
