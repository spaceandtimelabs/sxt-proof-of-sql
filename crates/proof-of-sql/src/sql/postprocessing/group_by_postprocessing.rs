...
mod tests {
    ...
    fn we_can_get_free_identifiers_from_expr() {
        ...
        // a + b + 1
        let expr = add(add(col("a"), col("b")), lit(1));
        let expected: IndexSet<Ident> =
            [ident("a").into(), ident("b").into()].into_iter().collect();
        let actual = get_free_identifiers_from_expr(&expr);
        assert_eq!(actual, expected);

        // ! (a == b || c >= a)
        let expr = not(or(equal(col("a"), col("b")), ge(col("c"), col("a"))));
        let expected: IndexSet<Ident> = [ident("a").into(), ident("b").into(), ident("c").into()]
            .into_iter()
            .collect();
        let actual = get_free_identifiers_from_expr(&expr);
        assert_eq!(actual, expected);
        ...
        let expected: IndexSet<Ident> =
            [ident("c").into(), ident("d").into()].into_iter().collect();
        let actual = get_free_identifiers_from_expr(&expr);
        assert_eq!(actual, expected);
    }
