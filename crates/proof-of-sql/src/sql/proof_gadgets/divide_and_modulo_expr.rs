...
impl DivideAndModuloExpr {
    ...
    pub fn verifier_evaluate<S: Scalar>(
        ...
        let sqrt_min_plus_s =
            verifier_evaluate_sign::<_, 128>(builder, min_sqrt_eval + s, one_eval)?;
        let sqrt_min_less_s =
            verifier_evaluate_sign::<_, 128>(builder, min_sqrt_eval - s, one_eval)?;
        ...
        verifier_evaluate_sign::<_, 129>(builder, quotient, one_eval)?;

        // sign(a) * r = sign(r) * r and sign(r - b) * b + sign(r + b) * b = b
        let lhs_sign = verifier_evaluate_sign::<_, 128>(builder, lhs, one_eval)?;
        let remainder_sign = verifier_evaluate_sign::<_, 128>(builder, remainder, one_eval)?;

        let remainder_and_rhs_difference_sign =
            verifier_evaluate_sign::<_, 129>(builder, remainder - rhs, one_eval)?;
