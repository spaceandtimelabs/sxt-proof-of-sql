    ...
    SliceExec<C>: ProverEvaluate<C::Scalar>,
{
    ...
    fn verifier_evaluate(
        ...
        let columns_evals = self
            .input
            .verifier_evaluate(builder, accessor, None)?;
        // 3. indexes
        ...
        let filtered_columns_evals =
            Vec::from_iter(repeat_with(|| builder.consume_result_mle()).take(columns_evals.len()));
...
impl<C: Commitment> ProverEvaluate<C::Scalar> for SliceExec<C> {
    ...
    fn result_evaluate<'a>(
        ...
        let input_num_rows = if columns.len() == 0 {
            0
        } else {
            columns[0].len()
        };
