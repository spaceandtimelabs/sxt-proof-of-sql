...
use crate::{
    base::{
        database::{
            owned_table_utility::*, Column, ColumnField, ColumnType, OwnedTable,
...
fn we_can_get_an_empty_result_from_a_slice_on_an_empty_table_using_result_evaluate() {
    ...
    let res = ProvableQueryResult::new(output_length as u64, &result_cols)
...
fn we_can_prove_a_slice_with_empty_results() {
    ...
    let ast = slice_exec(
        filter(cols_expr_plan(t, &["b"], &accessor), tab(t), where_clause),
        1,
        Some(2),
    );
    let verifiable_res = VerifiableQueryResult::new(&ast, &accessor, &());
