//! This module contains the conversion functions from `sqlparser::ast` to PoSQL Logical Plan.
use super::{BinaryOperator, LogicalPlanError, UnaryOperator};
use sqlparser::ast;
...
impl TryFrom<ast::UnaryOperator> for UnaryOperator {
    ...
    fn try_from(op: ast::UnaryOperator) -> Result<Self, Self::Error> {
        match op {
            ...
            _ => Err(LogicalPlanError::UnsupportedUnaryOperator { op }),
        }
    }
}
...
mod tests {
    ...
    // Unary operators
    #[test]
    fn we_can_convert_supported_sqlparser_unary_operators() {
        // Let's test all our supported unary operators.
        let test_cases = vec![(ast::UnaryOperator::Not, UnaryOperator::Not)];

        for (sql_op, expected) in test_cases {
            let result = UnaryOperator::try_from(sql_op).unwrap();
            assert_eq!(result, expected);
        }
    }

    #[test]
    fn we_cannot_convert_unsupported_sqlparser_unary_operators() {
        // Let's test an unsupported operator.
        let unsupported_op = ast::UnaryOperator::PGSquareRoot;
        let result = UnaryOperator::try_from(unsupported_op);
        assert!(matches!(
            result,
            Err(LogicalPlanError::UnsupportedUnaryOperator { .. })
        ));
    }
}
