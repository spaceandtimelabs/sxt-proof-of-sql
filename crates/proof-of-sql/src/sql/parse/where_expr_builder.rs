...
use crate::{
    ...
    sql::ast::{ProvableExpr, DynProofExpr},
};
use indexmap::IndexMap;
use proof_of_sql_parser::{intermediate_ast::Expression, Identifier};

/// Builder that enables building a `proof_of_sql::sql::ast::DynProofExpr` from a `proof_of_sql_parser::intermediate_ast::Expression` that is
...
impl<'a> WhereExprBuilder<'a> {
    ...
    /// Builds a `proof_of_sql::sql::ast::DynProofExpr` from a `proof_of_sql_parser::intermediate_ast::Expression` that is
    /// intended to be used as the where clause in a filter expression or group by expression.
