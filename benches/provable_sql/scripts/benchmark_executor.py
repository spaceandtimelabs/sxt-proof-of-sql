...
class BenchmarkExecutor:
    def __init__(self, args):
        ...
        self.exported_libs = (
            "LD_LIBRARY_PATH='"
            + run_process(
                "echo $(pwd)/$(dirname $(ci/find_lib.sh target/release/build/ libblitzar.so))/"
            )[:-1]
            + "'"
        )

    def build_bench_setting(
        self, query_idx, where_expr, num_result_cols, num_table_cols
    ):
    ...
    def process_all_queries(self):
        ...
        clauses = [
            ...
            (
                self.args.num_result_columns,
                self.args.num_table_columns,
                "(A = 2) and (B = 3)",
            ),
            (
                self.args.num_result_columns,
            ...
            (
                self.args.num_result_columns,
                self.args.num_table_columns,
                "((C = 0) or (B = 1)) and (not (A = -1))",
            ),
            (
                self.args.num_result_columns,
                self.args.num_table_columns,
                "((C = 0) and (B = 1)) and (not (A = -1))",
            ),
            (
                self.args.num_result_columns,
                self.args.num_table_columns,
                "((C = 0) and (B = 1)) or (not (A = -1))",
            ),
            (
                self.args.num_result_columns,
                self.args.num_table_columns,
                "((C = 0) or (B = 1)) or (not (A = -1))",
            ),
        ]
    ...
    def run_multi_rows_benchmark(self, bench):
        if self.args.generate_plots:
            ...
            for curr_table_length in bench.table_lengths():
                ...
                run_cmd = (
                    self.exported_libs + " " + self.binary_file + " " + curr_params
                )
            ...
            with open(
                self.args.output_dir + bench.plot_execution_times_file(), "w"
            ) as fp:
                fp.write("\n".join(str(ex) for ex in execution_times))

            print(
                "Finished processing multi row benchmark query %d: %s"
                % (bench.idx(), bench.select_statement())
            )
        else:
    ...
    def run_callgrind_benchmark(self, bench):
        if self.args.generate_callgrind:
            ...
            run_cmd = (
                self.exported_libs
                + " valgrind --callgrind-out-file="
                + output_dir
                + "/"
                + bench.callgrind_out_file()
                + " --tool=callgrind --dump-instr=yes --collect-jumps=yes --simulate-cache=yes --collect-atstart=no "
                + self.binary_file
                + " "
                + curr_params
            )
            ...
            print(
                "Finished processing callgrind benchmark query %d: %s"
                % (bench.idx(), bench.select_statement())
            )
    ...
    def run_benchmarks(self):
        ...
        generate_all_plots_queries(
            self.args.output_dir, all_benches, self.all_plots_data_file
        )
