...
def create_statistical_data(statistics_file, all_benches):
    with open(statistics_file, 'w') as fs:
        ...
        fs.write('ram (GB): ' + str(round(psutil.virtual_memory().total / (1024.0 **3))) + '\n')
        ...
        for bench_idx, bench in enumerate(all_benches):
            ...
            fs.write('number of table columns: ' + str(bench.num_table_columns()) + '\n')
            fs.write('number of result columns: ' + str(bench.num_result_columns()) + '\n')
            fs.write('callgrind file path: ' + bench.callgrind_svg_file() + '\n')
            fs.write('callgrind table length: ' + str(bench.callgrind_table_length()) + '\n')
            fs.write('callgrind number samples: ' + str(bench.num_callgrind_samples()) + '\n')
            fs.write('all plots number samples: ' + str(bench.num_plot_samples()) + '\n')
            fs.write('all plots table length: ' + ' '.join([str(l) for l in bench.table_lengths()]) + '\n')
            fs.write('all plots execution time (ms): ' + ' '.join([str(float(ex)) for ex in bench.execution_times()]) + '\n\n')
    ...
    if os.path.isfile(statistics_file):
        with open(statistics_file, 'r') as fs:
            ...
            for bench_idx, bench in enumerate(benches):
                ...
                ref_plots_table_length = [int(val) for val in fs.readline().split(': ')[1][:-1].split(' ')]
                ref_plots_exec_times = [float(ex) for ex in fs.readline().split(': ')[1][:-1].split(' ')]
                ...
                if ref_query_idx != bench_idx:
                    raise Exception("Ref query index %d differ from %d" % (ref_query_idx, bench_idx))
                elif ref_query_string != bench.select_statement():
                    raise Exception("Ref Query string `%s` differ from `%s`" % (ref_query_string, bench.select_statement()))
                elif math.fabs(ref_min_value_table - bench.min_value_table()) > 1e-3:
                    raise Exception("Ref min value table %f differ from %f" % (ref_min_value_table, bench.min_value_table()))
                elif math.fabs(ref_max_value_table - bench.max_value_table()) > 1e-3:
                    raise Exception("Ref max value table %f differ from %f" % (ref_max_value_table, bench.max_value_table()))
                elif ref_number_table_columns != bench.num_table_columns():
                    raise Exception("Ref number table columns %d differ from %d" % (ref_number_table_columns, bench.num_table_columns()))
                elif ref_number_result_columns != bench.num_result_columns():
                    raise Exception("Ref number result columns %d differ from %d" % (ref_number_result_columns, bench.num_result_columns()))
                elif ref_callgrind_table_length != bench.callgrind_table_length():
                    raise Exception("Ref callgrind table length %d differ from %d" % (ref_callgrind_table_length, bench.callgrind_table_length())) 
                elif ref_callgrind_num_samples != bench.num_callgrind_samples():
                    raise Exception("Ref callgrind number of samples %d differ from %d" % (ref_callgrind_num_samples, bench.num_callgrind_samples()))
                elif ref_plots_num_samples != bench.num_plot_samples():
                    raise Exception("Ref all plots number of samples %d differ from %d" % (ref_plots_num_samples, bench.num_plot_samples()))
                elif ref_plots_table_length != bench.table_lengths():
                    raise Exception("Ref all plots table lengths %d differ from %d" % (ref_plots_table_length, bench.table_lengths()))
                elif len(ref_plots_exec_times) != len(bench.table_lengths()):
                    raise Exception("Ref execution time length %d differ from %d" % (len(ref_plots_exec_times), len(bench.table_lengths())))

                all_execution_times.append(ref_plots_exec_times)

    return all_execution_times

def generate_and_copy_assets(output_dir, ref_statistics_dir, statistics_data_file, tar_tgz_data_file, all_benches):
    ...
    all_execution_times = get_statistical_data(output_dir + '/' + statistics_data_file, all_benches)
    ...
    run_process('cp ' + ref_statistical_file + ' ' + output_dir + '/ref_' + statistics_data_file)

    for count, bench in enumerate(all_benches):
        # we also copy the reference callgrind svg files
        run_process('cp ' + ref_statistics_dir + '/' + bench.callgrind_svg_file() + ' ' + output_dir + '/' + bench.ref_callgrind_svg_file())
    ...
    run_process('tar -cvzf ' + output_dir + '/' + tar_tgz_data_file + ' ' + files_to_save_as_tgz)
